...
norm_dict = {0: [2.3, 0.24], 1: [2.29, 0.22], 2: [2.29, 0.23], 3: [2.31, 0.24], 4: [2.31, 0.24],
             ...
             20: [2.21, 0.08], 21: [2.21, 0.08], 22: [2.21, 0.08], 23: [2.21, 0.08], 24: [2.21, 0.08],
             25: [2.21, 0.08], 26: [2.21, 0.08], 27: [2.21, 0.08], 28: [2.21, 0.08], 29: [2.21, 0.08],
             30: [2.21, 0.08], 31: [2.21, 0.08], 32: [2.21, 0.08], 33: [2.21, 0.08], 34: [2.21, 0.08],
             35: [2.21, 0.08], 36: [2.21, 0.08], 37: [2.21, 0.08], 38: [2.21, 0.08], 39: [2.21, 0.08],
             40: [2.21, 0.08], 41: [2.21, 0.08], 42: [2.21, 0.08], 43: [2.21, 0.08], 44: [2.21, 0.08],
             45: [2.21, 0.08], 46: [2.21, 0.08], 47: [2.21, 0.08], 48: [2.21, 0.08], 49: [2.21, 0.08]}
model_pka_dict = {
...
ACIDIC_MODEL_DIR      = "env/wts/esm2/acidic"
BASIC_MODEL_DIR       = "env/wts/esm2/basic"
CBTREE_MODEL_DIR      = "env/wts/CBtree2"
ACIDIC_MODEL_DIR_ESM2 = "env/wts/esm2/acidic"
ACIDIC_MODEL_DIR_ESMC = "env/wts/esmC/acidic"
...
def parse_args():
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Directory of precomputed PDB structures; "
            "will look for <unique_id>.pdb here before folding/fetching"
        ),
    )
    p.add_argument(
        "--nofold",
        action="store_true",
        help="Skip structure folding step"
    )
    p.add_argument(
        "--outdir",
        type=str,
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="Threads per sequence for ensemble inference"
    )
    p.add_argument(
        "--debug",
        action="store_true",
...
def get_struct_from_dir(unique_id, structs_dir, out_dir):
    """
    Check for a PDB file named <unique_id>.pdb in structs_dir.
    If found, copy it into out_dir and return its new path; otherwise return None.
    """
    import os
    import shutil
    import logging

    src = os.path.join(structs_dir, f"{unique_id}.pdb")
    if os.path.isfile(src):
        dest = os.path.join(out_dir, f"{unique_id}.pdb")
        try:
            shutil.copy(src, dest)
            logging.info(
                "Using provided structure for %s from %s",
                unique_id, src
            )
            return dest
        except Exception as e:
            logging.error(
                "Failed to copy structure for %s from %s: %s",
                unique_id, src, e
            )
    return None

def get_titratable_indices(residue_info, channel):
    # Handle both one‑letter (no‑fold) and three‑letter (PDB) codes
    idxs = []
    for i, (res, rn, ch, si) in enumerate(residue_info):
        # if res is already one‑letter, use it; else map three‑letter -> one‑letter
        if len(res) == 1:
            aa = res.upper()
        else:
            aa = THREE_TO_ONE.get(res.upper(), "X")
...
def old_get_titratable_indices(residue_info, channel):
    idxs = []
    for i, (res, rn, ch, si) in enumerate(residue_info):
        aa = THREE_TO_ONE.get(res.upper(), "X")
        if channel == "acidic" and aa in residue_groups["acidic"]:
...
def write_predictions_csv(predictions, out_csv):
    ...
    with open(out_csv, "w") as f:
        ...
        for uid, vals in predictions.items():
            # map one-letter code → three-letter residue name
            parts = uid.split("_", 2)
            three = ONE_TO_THREE.get(parts[0], parts[0])
            uid3 = "_".join([three, parts[1], parts[2]])
            if len(vals) == 4:
...
def old_write_predictions_csv(predictions, out_csv):
    import logging
    include_cbtree = any(len(v) == 4 for v in predictions.values())
    if include_cbtree:
        headers = [
...
def process_single_pipeline(
    ...
    # 1) If --structs provided, try to symlink <unique_id>.pdb -> folded_structure.pdb
    if args.structs:
        src = os.path.join(args.structs, f"{unique_id}.pdb")
        if os.path.isfile(src):
            link = os.path.join(out_dir, "folded_structure.pdb")
            try:
                os.symlink(os.path.abspath(src), link)
                logging.info(
                    "Symlinked precomputed structure for %s -> %s",
                    unique_id, link
                )
            except FileExistsError:
                logging.debug("Symlink already present: %s", link)
            provided_structure_file = link
        else:
            logging.info(
                "No precomputed %s.pdb in %s; will fold/fetch",
                unique_id, args.structs
            )

    # 2) If we now have any PDB on disk, split chains & run inference
    if provided_structure_file and provided_structure_file.lower().endswith(".pdb"):
        chain_files = split_pdb_by_chain(provided_structure_file, out_dir)
        if len(chain_files) > 1:
            logging.warning(
                "Multiple chains found; processing each separately."
            )
        ...
        for chain, chain_file in chain_files.items():
            ...
            # embeddings
            emb_acidic, acidic_ch = get_acidic_embedding(
                seq_chain, chain_header, args.acidic
            )
            if args.basic == "esm2":
                from __main__ import extract_esm2_representation
                outb = extract_esm2_representation(seq_chain, chain_header)
                emb_basic = outb["representations"][ESM2_REPR[0]]
            else:
                from __main__ import extract_esmc_representation
                outb = extract_esmc_representation(seq_chain, chain_header)
                emb_basic = outb["representations"][ESMC_REPR[0]]

            # titratable indices
            a_idx = get_titratable_indices(residue_info, "acidic")
            a_info = [residue_info[i] for i in a_idx]
            b_idx = get_titratable_indices(residue_info, "basic")
            b_info = [residue_info[i] for i in b_idx]
            emb_a = emb_acidic[a_idx]
            emb_b = emb_basic[b_idx]

            # run ensembles
            logging.info("Running ensemble: acidic (chain %s)...", chain)
            preds_a = run_acidic_inference(
                emb_a, a_info, acidic_ch, args.nthreads
            )
            logging.info("Running ensemble: basic (chain %s)...", chain)
            from __main__ import PRELOADED_BASIC_WEIGHTS
            preds_b = run_ensemble_inference(
                emb_b, b_info, "basic", args.nthreads,
                PRELOADED_BASIC_WEIGHTS,
            )
            ...
            for res in residue_info:
                rname, rseq, ch_id, seq_i = res
                uid = f"{rname}_{seq_i}_{ch_id}"
                if uid in preds_a:
            ...
            if not args.nocbtree:
                cbt = run_cb_tree_channel(
                    chain_file, out_dir, unique_id + f"_{chain}"
                )
                for uid, val in final_preds.items():
                    final_preds[uid] = (*val, cbt.get(uid, 0.0))

            # update per-chain PDB
            updated = chain_file + ".upd.pdb"
            update_pdb_beta_factors(chain_file, final_preds, updated)
            updated_chain_files.append(updated)
            combined_preds.update(final_preds)

        # write out combined CSV + PDB
        ...
        logging.info("Pipeline done for %s using PDB.", unique_id)
        return

    # 3) No PDB on disk: fold via Forge (or skip if --nofold)
    residue_info = None
    if not provided_structure_file and not args.nofold:
        folded = forge_fold_structure(sequence, header, out_dir)
        provided_structure_file = folded
        seq_from_pdb, residue_info = parse_pdb_to_sequence(folded)
        if len(seq_from_pdb) != len(sequence):
            logging.warning("Folded structure length ≠ input sequence!")

    # 4) If still no residue_info (e.g. --nofold + no PDB), fake it from seq
    ...
    # 5) Single-chain inference on folded/fetched structure
    emb_acidic, acidic_ch = get_acidic_embedding(
        sequence, header, args.acidic
    )
    if args.basic == "esm2":
        from __main__ import extract_esm2_representation
        outb = extract_esm2_representation(sequence, header)
        emb_basic = outb["representations"][ESM2_REPR[0]]
    else:
        from __main__ import extract_esmc_representation
        outb = extract_esmc_representation(sequence, header)
        emb_basic = outb["representations"][ESMC_REPR[0]]
    ...
    # merge, optional CBTREE, write CSV & PDB
    final_preds = {}
    for res in residue_info:
        rname, rseq, ch, seq_i = res
        uid = f"{rname}_{seq_i}_{ch}"
        if uid in preds_a:
            final_preds[uid] = preds_a[uid]
        elif uid in preds_b:
            final_preds[uid] = preds_b[uid]
    ...
    if not args.nocbtree and provided_structure_file:
        cbt = run_cb_tree_channel(provided_structure_file, out_dir, unique_id)
        for uid, val in final_preds.items():
            final_preds[uid] = (*val, cbt.get(uid, 0.0))

    csv_out = os.path.join(out_dir, "predictions.csv")
    write_predictions_csv(final_preds, csv_out)
    if provided_structure_file:
